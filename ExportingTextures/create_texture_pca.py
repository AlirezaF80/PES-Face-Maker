...
# Save reconstructed textures and calculate errors
os.makedirs(os.path.join(BASE_PATH, "reconstructed_textures"), exist_ok=True)
...
for i, texture in enumerate(reconstructed_textures):
    texture_path = os.path.join(BASE_PATH, "reconstructed_textures", f"reconstructed_texture_{i}.png")
    cv2.imwrite(texture_path, texture.astype(np.uint8))
