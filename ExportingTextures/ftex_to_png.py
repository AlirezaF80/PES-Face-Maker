...
def ddsMipmapSize(ftexFormat, width, height, depth, mipmapIndex):
    ...
    if ftexFormat not in formatBlockConfiguration:
        raise DecodeError(f"Unsupported ftex format: {ftexFormat}")
    (blockSizePixels, blockSizeBytes) = formatBlockConfiguration[ftexFormat]
    scaleFactor = 2 ** mipmapIndex

    mipmapWidth = max(1, width // scaleFactor)
    mipmapHeight = max(1, height // scaleFactor)
    mipmapDepth = max(1, depth // scaleFactor)
...
def ftexToDds(ftexFilename, ddsFilename):
    with open(ftexFilename, 'rb') as inputStream:
        inputBuffer = inputStream.read()

    try:
        outputBuffer = ftexToDdsBuffer(inputBuffer)
    except DecodeError as e:
        print(f"Error converting '{ftexFilename}' to DDS: {e}")
        sys.exit(1)

    with open(ddsFilename, 'wb') as outputStream:
        outputStream.write(outputBuffer)
    print(f"Converted '{ftexFilename}' to '{ddsFilename}' successfully.")
...
def ddsToFtexBuffer(ddsBuffer, colorSpace):
    ...
    if ddsFormatFlags & 0x4 == 0:  # fourCC absent
        ...
        if ddsExtensionFormat == 61:  # DXGI_FORMAT_R8_UNORM
            ...
            ftexPixelFormat = 12
        elif ddsExtensionFormat == 2:   # DXGI_FORMAT_R32G32B32A32_FLOAT
            ftexPixelFormat = 13
...
def ddsToFtex(ddsFilename, ftexFilename, colorSpace='SRGB'):
    with open(ddsFilename, 'rb') as inputStream:
        inputBuffer = inputStream.read()

    try:
        outputBuffer = ddsToFtexBuffer(inputBuffer, colorSpace)
    except DecodeError as e:
        print(f"Error converting '{ddsFilename}' to FTEX: {e}")
        sys.exit(1)

    with open(ftexFilename, 'wb') as outputStream:
        outputStream.write(outputBuffer)
    print(f"Converted '{ddsFilename}' to '{ftexFilename}' successfully.")
...
def get_random_id(imagePath):
    """
    Used to generate a random ID (valid filename) for the temp file
    """
    return hashlib.md5(imagePath.encode()).hexdigest()

def main():
    if len(sys.argv) < 2:
        print("Usage:")
        print("  To convert FTEX to PNG:")
        print("    python ftex_to_png.py <input.ftex> <output.png>")
        sys.exit(1)
    ...
    if not os.path.isfile(inputFilename):
        print(f"Input FTEX file '{inputFilename}' does not exist.")
        sys.exit(1)

    tempDdsFilename = f"temp_{get_random_id(inputFilename)}.dds"
    try:
        ftexToDds(inputFilename, tempDdsFilename)
        image = Image.open(tempDdsFilename)
        image = image.convert('RGB')
        image.save(outputFilename, 'PNG')
        print(f"Converted '{inputFilename}' to '{outputFilename}' successfully.")
    except DecodeError as e:
        ...
        if os.path.isfile(tempDdsFilename):
            os.remove(tempDdsFilename)
